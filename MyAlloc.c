...
void mark( ClassInstance *obj ) {
    ...
    if !(isMarked(obj)) {
        setMarked(obj);
        for (ptr = obj + 4; ptr < (size + obj); ptr++) {
            if (conservativeClass(ptr)) {
                mark(ptr);
            }
        }
    }
...
int isMarked( ClassInstance *obj ) {
    return (uint32_t)*obj & 0x80000000;
}

void setMarked ( ClassInstance *obj ) {
    (uint32_t)*obj |= 0x80000000;
}

void unsetMarked ( ClassInstance *obj ) {
    (uint32_t)*obj &= 0x7fffffff;
}

int realSize( ClassInstance *obj) {
    return (uint32_t)*obj & 0x7fffffff;
}
